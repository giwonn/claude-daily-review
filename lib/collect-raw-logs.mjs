...
import { getISOWeek, getISOWeekYear, getQuarter, formatDate } from './periods.mjs';
...
async function main() {
  ...
  // Always include current in-progress periods so cascading summaries stay up-to-date
  const today = new Date();
  if (affectedDates.size > 0) {
    if (periods.weekly) affectedWeeks.add(toWeekKey(today));
    if (periods.monthly) affectedMonths.add(toMonthKey(today));
    if (periods.quarterly) affectedQuarters.add(toQuarterKey(today));
    if (periods.yearly) affectedYears.add(toYearKey(today));
  }
