...
async function main() {
  ...
  // 3. Determine which dailies need generation
  const allDates = Object.keys(logsByDate).sort();
  const needsDaily = allDates.filter(d => !existingDaily.has(d));
  ...
  const needsWeekly = periods.weekly ? [...weeksSeen].filter(w => !existingWeekly.has(w)).sort() : [];
  const needsMonthly = periods.monthly ? [...monthsSeen].filter(m => !existingMonthly.has(m)).sort() : [];
  const needsQuarterly = periods.quarterly ? [...quartersSeen].filter(q => !existingQuarterly.has(q)).sort() : [];
  const needsYearly = periods.yearly ? [...yearsSeen].filter(y => !existingYearly.has(y)).sort() : [];
