...
import { getRawDir, getRawLogPath } from './vault.mjs';
...
/** @param {StorageAdapter} storage @param {string} date @param {HookInput} entry @returns {Promise<void>} */
export async function appendRawLog(storage, date, entry) {
  const dir = getRawDir(date);
  await storage.mkdir(dir);
  const logPath = getRawLogPath(date, entry.session_id);
 ...
 */
export async function appendGitLogs(storage, date, gitEntries, sessionId, ghAccount) {
